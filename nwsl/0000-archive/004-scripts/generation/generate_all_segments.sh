#!/bin/bash
# Generate all 9 NWSL segments with full canon prompts
set -e
...
echo "🎬 NWSL 9-SEGMENT GENERATION PIPELINE"
echo "====================================="
...
extract_prompt() {
    local canon_file=$1
    # Extract content between --- markers, before any metadata sections
    awk '
...
EOF
    ...
    OPERATION=$(echo "$RESPONSE" | jq -r '.name // .error.message')
